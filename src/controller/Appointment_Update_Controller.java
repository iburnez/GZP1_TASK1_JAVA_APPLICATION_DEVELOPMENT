...
public class Appointment_Update_Controller implements Initializable {
    ...
    /**
     * _____________________________________________________________________
     *                     SAVE UPDATE APPOINTMENT METHOD
     * ---------------------------------------------------------------------
     */

    /**
     * SAVE/UPDATE APPOINTMENT                                                                          <br>
    ...
    void onActionSave(ActionEvent event) throws IOException {
        ...
        if(validateFields()) {
            ...
            if(businessHoursCheck(start, end)) {
                if(checkAllApptOverlap(start, end, appointmentID)){
     ...
     * @return true/false form field are empty  <br>
     */
     ...
     */
    boolean checkAllApptOverlap(String start, String end, int ApptID){
        ...
        try {
            ...
            while(rs.next()){
                ...
                //Do not compare new appointment to itself
    ...
    /**
     * SEND APPOINTMENT FROM MAIN MENU                                                  <br>
     * Display appointment info in appropriate form fields                              <br>
     * Calculate LocalDateTime from UTC for appointment start and end time              <br>
     * Calculate duration of appointment for appointment duration form comboxes.        <br>
     * @param appointment get appointment param from selected appointment on main menu  <br>
     */
    public void sendAppointment(Appointment appointment){
        //Get customer object for appointment
        Customer apptCust = Customer.getCustomerByID(appointment.getCustomerID());

        //Parse date from appointment start variable
        String apptDate = TimeConverter.parseDate(appointment.getStart());

        //Get Duration from StartTime - EndTime (UTC used for duration calcs due to 24 hour clock)
        //Parse Hours/Mins from Appointment Duration
        String durationHours = TimeConverter.getDurationHours(TimeConverter.parseTime(appointment.getStartUTC()), TimeConverter.parseTime(appointment.getEndUTC()));
        String durationMin = TimeConverter.getDurationMins(TimeConverter.parseTime(appointment.getStartUTC()), TimeConverter.parseTime(appointment.getEndUTC()));;

        //Set form field values using Appointment variables
        ...
        appointmentURLTxt.setText(appointment.getAppointmentURL());
        custNameTxt.setItems(Customer.getAllCustomers());
        custNameTxt.getSelectionModel().select(apptCust);
        appointmentDatePicker.setValue(LocalDate.parse(apptDate));

        //Use BusinessTime methods to locate object and populate form fields with apprporiate data from Appointment
        selectHourCmbBx.getSelectionModel().select(BusinessTime.getbHoursByMath(TimeConverter.parseHour(appointment.getStart())));
        selectMinuteCmbBx.getSelectionModel().select(BusinessTime.getbMinsByMath(TimeConverter.parseMins(appointment.getStart())));
        durationHoursCmbBx.getSelectionModel().select(BusinessTime.getDhourByMath(durationHours));
        durationMinutesCmbBx.getSelectionModel().select(BusinessTime.getDMinByMath(durationMin));
    }

    /**
     * INITIALIZE COMBOX DROPDOWN MENUS                                                     <br>
